...
def test_legacy_runtime_identifiers_remain_for_upgrades():
    ...
    installer_text = (ROOT / "packaging/install-linux.sh").read_text()
    assert "mesh-wx.service" in installer_text
    assert "mesh-wx-guest.service" in installer_text
    assert 'if [ -f /etc/mesh-wx/guest-password ]; then' in installer_text
    ...
    assert "ProtectHome=true" not in guest_service
    assert "NoNewPrivileges=true" in guest_service
